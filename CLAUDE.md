...
- **Settings:** `/admin/settings` — 7 tabs in 3 groups: **Truffels** [Services (restart loop detection, dependent service handling, admission control), Alerts (temp thresholds), Updates (update check interval 1–168h, default 24h)], **Host** [Info (hardware/OS/network), Logs (journalctl viewer with unit/severity/boot filtering), Tuning (persistent journal, swappiness)], **Danger Zone** (system shutdown/restart with password confirmation)
...
- **CI:** GitHub Actions — 3 parallel jobs (API Go tests, Agent Go tests, Web Vitest), golangci-lint (errcheck), tsc --noEmit, coverage reporting, 413 tests total
