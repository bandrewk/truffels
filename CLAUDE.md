...
- **Updates:** Automatic version checking (Docker Hub / Docker Digest / GitHub / Bitbucket / GitHub Releases) for 11 services (bitcoind, electrs, mempool, mempool-db, ckpool, ckstats, proxy/Caddy, ckstats-db/PostgreSQL, truffels-agent, truffels-api, truffels-web), tag filter support, preflight checks, one-click apply with automatic rollback, pull & restart for floating-tag services (MariaDB), self-update for truffels services (git checkout + rebuild + detached restart via nsenter), configurable background check cycle (default 24h, 1–168h via Settings)
...
- **Settings:** `/admin/settings` — 7 tabs in 3 groups: **Truffels** [Services (restart loop detection, dependent service handling, admission control), Alerts (temp thresholds), Updates (auto-check toggle + interval 1–168h, default 24h)], **Host** [Info (hardware model/CPU/RAM, OS/kernel/uptime, storage mounts with usage, network interfaces with IP/MAC), Logs (journalctl viewer with unit/severity/boot filtering), Tuning (persistent journal, swappiness)], **Danger Zone** (system shutdown/restart with password confirmation)
