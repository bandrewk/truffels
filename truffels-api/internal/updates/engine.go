...
func (e *Engine) ApplyUpdate(serviceID string) error {
	if IsCatalogService(serviceID) {
		return fmt.Errorf("%s is a catalog service: updates run through the catalog, not through RewriteTags", serviceID)
	}
...
// Catalog services must not use the legacy update path.
//
// The engine currently updates by using RewriteTags to replace image tags in
// the compose file (see engine.go:560 and the rollback paths). For a catalog
// service, this is wrong: the file is generated by the agent from the catalog,
// any changes to it would be gone on the next Apply — and it would bypass the
// typed generator that carries the security property.
//
// An update to a catalog service is instead: change the pinned version in the
// catalog entry, release it, and call /v1/service/apply again.
var (
	catalogIDsMu sync.RWMutex
	catalogIDs   map[string]bool
)

// SetCatalogIDs is called at startup from the catalog client.
...
func IsCatalogService(id string) bool {
	catalogIDsMu.RLock()
	defer catalogIDsMu.RUnlock()
	return catalogIDs[id]
}
