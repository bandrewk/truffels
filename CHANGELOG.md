...
### Fixed
- Self-update build failure: `docker compose build` now runs via nsenter so build-context paths resolve on the host filesystem
- Self-update git error: added `safe.directory /repo` to agent Dockerfile so git operations work inside the container
